...
function doesChildVersionMatch (child, requested, requestor) {
  ...
  const links = getTop(child).links
  const linkInfo = links && links[moduleName(child)]
  if (linkInfo) {
    return child.isLink && linkInfo.realpath === child.realpath
  }
...
function computeVersionSpec (tree, child) {
  ...
  var requested
  var childReq = child.package._requested
  if (childReq.type === 'directory' && child.isLink) {
